...
import static com.binance.api.log.LogManager.println;
...
//                println(newEvent);
        ...
        final Consumer<DepthEvent> drainPending = newEvent -> {
            ...
            // after the snapshot.
            pendingDeltas.stream().filter(e -> e.getFinalUpdateId() > lastUpdateId) // Ignore any updates before the snapshot
    ...
    private static final Logger logger = LoggerFactory.getLogger(DepthCacheExample.class);
...
//        println(depthCache);
        println("ASKS:(" + getAsks().size() + ")");
        double askMax = 0;
        double askMaxPrice = 0;
        double askTotal = 0;
        double bidMax = 0;
      double bidMaxPrice = 0;
        double bidTotal = 0;
        for (Entry<BigDecimal, BigDecimal> entry : getAsks().entrySet()) {
            //      println(toDepthCacheEntryString(entry));
          if (askMax < entry.getValue().doubleValue()) {

            askMax = entry.getValue().doubleValue();
            askMaxPrice = entry.getKey().doubleValue();
          }
            askTotal += entry.getValue().doubleValue();
        }

        println("BIDS:(" + getBids().size() + ")");
        for (Entry<BigDecimal, BigDecimal> entry : getBids().entrySet()) {
            //      println(toDepthCacheEntryString(entry));
          if (bidMax < entry.getValue().doubleValue()) {
            bidMax = entry.getValue().doubleValue();
            bidMaxPrice = entry.getKey().doubleValue();
          }

            bidTotal += entry.getValue().doubleValue();
        }

        logger.debug("printDepthCache askMax:{}, askMaxPrice:{}, bidMax:{},bidMaxPrice:{}", askMax,askMaxPrice, bidMax,bidMaxPrice);
        logger.debug("printDepthCache askTotal:{},bidTotal:{}", askTotal, bidTotal);
        println("BEST ASK: " + toDepthCacheEntryString(getBestAsk()));
        println("BEST BID: " + toDepthCacheEntryString(getBestBid()));
    }
    ...
    private final class WsCallback implements BinanceApiCallback<DepthEvent> {
        ...
        public void onFailure(Throwable cause) {
            println("WS connection failed. Reconnecting. cause:" + cause.getMessage());
